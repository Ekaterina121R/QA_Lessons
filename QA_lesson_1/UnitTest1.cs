...
namespace QA_lesson_1
{
    ...
    public class MyFirstTest
    {
        ...
        public void AddDuck()
        {
            ...
            {
                if (element.GetAttribute("textContent") == "Pirate Duck") elemCount++;
            }
            ...
            // Заполняем General
            string imgPath = Path.GetFullPath("pirate_duck.jpg");

            driver.FindElement(By.XPath("//div[@id='tab-general']/table/tbody/tr[1]/td/label[1]/input")).Click();
            driver.FindElement(By.XPath("//div[@id='tab-general']/table/tbody/tr[2]/td/span/input")).SendKeys("Pirate Duck");
            ...
            // Проверим, что новый товар с нашим именем появился
            driver.Url = "http://localhost/litecart/admin/?app=catalog&doc=catalog&category_id=1";
            elements = driver.FindElements(By.XPath("//table[@class='dataTable']//tr//td//a"));
            int newElemCount = 0;
            foreach (IWebElement element in elements)
            {
                if (element.GetAttribute("textContent") == "Pirate Duck") newElemCount++;
            }
